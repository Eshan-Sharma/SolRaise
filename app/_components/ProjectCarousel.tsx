...
const ProjectCarousel: React.FC<CarouselProps> = ({ projects }) => {
    ...
    const handleSlide = useCallback((direction: 'next' | 'prev') => {
        ...
        setCurrentIndex((prevIndex) =>
            direction === 'next' ? prevIndex + projectsPerSlide : prevIndex - projectsPerSlide
        );
    ...
    }, [currentIndex, isTransitioning, extendedProjects.length, projectsPerSlide]);
    ...
    return (
        <div className="relative overflow-hidden">
            <div
                ...
                style={{ transform: `translateX(-${(currentIndex * 100) / projectsPerSlide}%)` }}
            >
                {extendedProjects.map((project, index) => (
                    <Card key={`${project.id}-${index}`} className="flex-shrink-0 w-1/3 bg-gray-900 overflow-hidden relative h-[400px] mx-2">

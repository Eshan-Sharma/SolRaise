...
const DonationModal: React.FC<DonationModalProps> = ({ project, isOpen, onClose }) => {
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[600px] bg-white text-gray-900 rounded-lg shadow-lg p-6">
                <DialogHeader className="space-y-4">
                    <DialogTitle className="text-3xl font-extrabold text-purple-700">{project.title}</DialogTitle>
                    <DialogDescription className="text-lg text-gray-600">Support this project and make a difference!</DialogDescription>
                </DialogHeader>
                <div className="py-6 space-y-6">
                    <Progress value={(project.raised / project.goal) * 100} className="w-full h-3 bg-gray-300 rounded-full" />
                ...
                <div className="space-y-6">
                    <div className="space-y-3">
                        <Label htmlFor="donationAmount" className="text-lg font-semibold text-gray-700">Donation Amount (SOL)</Label>
                        <Input
                            id="donationAmount"
                            type="number"
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        />
                        <Label htmlFor="matchDonation" className="text-lg font-semibold text-gray-700">Match Donation</Label>
                    </div>
                    {matchDonation && (
                        <div className="space-y-3">
                            <Label htmlFor="maxMatchAmount" className="text-lg font-semibold text-gray-700">Maximum Match Amount (SOL)</Label>
                            <Input
                                id="maxMatchAmount"
                                type="number"
                ...
                <DialogFooter className="mt-8">
                    <Button onClick={handleDonate} className="w-full bg-purple-600 hover:bg-purple-700 text-white py-3 text-lg font-semibold rounded-lg transition-all">
                        Make Donation
                    </Button>
                </DialogFooter>
            </DialogContent>
